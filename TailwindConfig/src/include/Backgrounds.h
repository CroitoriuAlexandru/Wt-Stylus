...
namespace Tailwind {
    ...
    public:
        ...
        boost::regex background_clip_regex = boost::regex("(?<!\\S)(!)?bg-clip-(border|padding|content|text)(?!\\w|-)");
        
        
        boost::regex background_origin_regex = boost::regex("(?<!\\S)(!)?bg-origin-(border|padding|content)(?!\\w|-)");
        boost::regex background_position_regex = boost::regex("(?<!\\S)(!)?bg-(bottom|center|left|left-bottom|left-top|right|right-bottom|right-top|top)(?!\\w|-)");
        boost::regex background_repeat_regex = boost::regex("(?<!\\S)(!)?bg-(repeat|no-repeat|repeat-x|repeat-y|repeat-round|repeat-space)(?!\\w|-)");
        boost::regex background_size_regex = boost::regex("(?<!\\S)(!)?bg-(auto|cover|contain)(?!\\w|-)");
        boost::regex background_image_regex = boost::regex("(?<!\\S)bg-(gradient-to-(tr|tl|br|bl|r|b|t|l)|none)(?!\\w|-)");

        // those are good but i will implement colors at the end
